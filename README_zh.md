...
#### 导出数据标签页（仅 User 模式）
- **群列表元数据**: 导出账号可访问的群组和频道，包括 ID、名称、类型、创建时间、公开链接、成员数、简介和管理员列表；权限不足的字段会记录告警，不中断整批导出
- **群消息记录**: 选择群组和闭区间起止时间，导出 JSON、CSV 或离线 HTML 归档；HTML 会下载为 ZIP，解压后打开 `index.html`，支持关键词（含发送者姓名、用户名和 ID）、日期筛选与分页，每页最多 2000 条。图片、文件、视频等以可读占位符保存，不下载媒体文件
- **回复关系**: HTML 中显示被回复消息的发送者、时间和内容摘要；原消息在本次归档内时可跨数据片定位，不在导出范围时会明确标记
- **定时导出任务**: 支持每小时、每天和每周执行；每次只导出上次成功位置之后的增量数据，失败不会推进游标
- **运行记录**: 查看处理条数、时间范围、输出路径和错误；手动导出完成后也可直接从浏览器下载

完整历史导出依赖 Telegram 用户会话，Bot Token 模式不开放此功能。若 `WEB_HOST` 对外监听，必须同时配置 `WEB_AUTH_USERNAME` 和 `WEB_AUTH_PASSWORD`，否则导出会被禁用。Telegram 已删除的消息无法恢复，已编辑消息导出当前版本。
